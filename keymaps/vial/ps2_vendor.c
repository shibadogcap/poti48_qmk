...
// Modified for poti48: Skip PS/2 on left side (no device connected)
...
// 左側判定用フラグ（ps2_host_init前に設定される）
static bool ps2_disabled = false;
...
void ps2_host_init(void) {
    // 左側(GP29=HIGH)ではPS/2を無効化して即リターン
    gpio_init(29);
    gpio_set_dir(29, GPIO_IN);
    gpio_pull_up(29);
    wait_us(100);
    if (gpio_get(29)) {  // HIGH = 左側
        ps2_disabled = true;
